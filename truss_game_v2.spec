...
#
#   pyinstaller truss_game_v2.spec
#
...
# than the `truss_game_v2.exe` that actually shipped, and passed
...
a = Analysis(
    ...
    pathex=[],
    binaries=[],
    # The model is loaded by filename at runtime, and truss_game_v2.py chdirs to
    # sys._MEIPASS when frozen, so shipping it at the bundle root is what makes
    # `Interpreter(model_path='truss_game_v2_model.tflite')` resolve.
    datas=[('truss_game_v2_model.tflite', '.')],
    hiddenimports=[],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
